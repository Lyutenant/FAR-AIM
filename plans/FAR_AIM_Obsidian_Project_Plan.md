...
# 37. Design — FAR Defined-Term Links

The FAR-side analogue of the AIM's glossary links (§12.2, Tier 2), decided
after Phase 9: FAR notes never link the Pilot/Controller Glossary because
14 CFR defines its own vocabulary and the two can differ (`NIGHT`,
`CEILING`, `AIR TAXI`). Instead a FAR note lists the definitions in force
for it that its text uses — Part 1's chapter-wide terms and the part's or
subpart's own definitions section.

## 37.1 Placement and ownership

- Rendered at build time from the canonical layer; no canonical JSON
  changes, no new source, no new notes. The section `## Defined Terms`
  follows `## Explicit Cross-References` and precedes `## Related
  (derived)` in FAR section and appendix notes.
- Links are **block links** to the definition itself
  (`[[1.1#^def-night|Night]] (§ 1.1)`), naming the defining section so a
  part-local override reads as such. Every `definition` block the FAR
  renderer emits carries a block id derived from its term, so curated
  notes may cite a definition too (`[[61.1#^def-cross-country-time]]`).
  A block id is markup, not wording (§32.3); Reading View hides it.
- The gate `data/links/part1-definitions-gate.json` (deny-only, reasons
  required, optional per-source `section`, stale entry fails the build)
  is human-maintained curation (§12.3) like the PCG glossary gate.

## 37.2 Sources and scope

- A section holding definition blocks is a source when its heading names
  definitions or its lead-in states a scope. The scope is the narrowest of
  `this chapter` / `this subchapter` / `this part` / `this subpart` in the
  text introducing the first definition (the paragraph holding it, else the
  text before it); `this section` means self-only and the section is
  skipped. Without a phrase: subpart A or no subpart ⇒ the part, else the
  subpart. Authoritative and deterministic — the section's own words.
- A document lists terms from every source covering it (same chapter /
  subchapter / part / part+subpart) except itself; a source never lists a
  wider definition of a term it defines. Chapters II, III and V get their
  own sources the same way.
- Precedence: the narrower scope wins at a shared alias (§ 1.1's "unless
  the context requires otherwise"); within a scope the label that *is* the
  alias wins, else the alias is dropped.

## 37.3 Recognition rules (`far_aim.links.definitions`)

- Label = term minus trailing punctuation. A parenthetical yields the
  CFR's `Term (ABBR)` forms as aliases (`Decision altitude`, `ATS route`,
  `NDB`/`ADF`).
- Abbreviations (no lowercase letters) match in capitals only and at three
  or more characters. Phrases match case-insensitively at word boundaries,
  singular or plural (§ 1.3(a)). Single defined words match — a
  definitions section is a controlled vocabulary — unless denied.
- Longest alias wins at a position. Duplicated term text (§ 1.1's
  *Synthetic vision* / *Synthetic vision system*) is told apart by the
  words before the definition's verb; an unresolved duplicate is numbered
  and never linked.
- Denied (initial gate, every source): `Instrument`, `Type:`, `Class:`,
  `Category:`, `Category A,`, `Category B,`, `FAA`.

## 37.4 Validation

- A gate entry naming no defined term (or a section that does not define
  it) fails the build.
- Every generated block link must land on a block id its target note
  carries (`_verify_plan`); zero broken anchors by construction.
- `validate` byte-compares the sections like every other generated byte.

## 37.5 Exit criteria

```text
✓ § 91.155 lists Night, Controlled airspace, Flight visibility … as links into § 1.1
✓ § 61.57 lists Aeronautical experience, Flight training … as links into § 61.1
✓ Part 139 notes link § 139.5's Airport, not § 1.1's
