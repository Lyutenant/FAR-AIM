---
id: "cfr-14-91.413"
type: "regulation"
citation: "14 CFR § 91.413"
title_number: 14
part: 91
section: "91.413"
source: "ecfr"
source_version: "2026-09-15"
canonical_hash: "sha256:994716aa4b3939c81581e0954f3de840349d2a20cece8dc982ce43a70cab18bf"
generated: true
title: "ATC transponder tests and inspections"
aliases:
  - "§ 91.413"
  - "14 CFR 91.413"
  - "ATC transponder tests and inspections"
tags:
  - "far"
  - "regulation"
cssclasses:
  - "far-aim-text"
---

# § 91.413 — ATC transponder tests and inspections

> [!info] Source
> eCFR Title 14, issue 2026-09-15 — [view on eCFR](https://www.ecfr.gov/on/2026-09-15/title-14/section-91.413)

> [!study] Private Pilot study aid — curated, not official text
> **Gist:** Any ATC transponder you use must have been tested and inspected within the preceding 24 calendar months, and re-tested after any installation or maintenance that could introduce a data error. Only a rated repair station, a continuous-airworthiness program holder or the manufacturer may do it.
> **Why it matters:** It is on the inspection list for every airplane with a transponder, VFR or IFR.
> **Numbers:** 24 calendar months
> **Mnemonic (training-community, not FAA):** AV1ATE: Annual, VOR (IFR, 30 days), 100-hour, Altimeter (24 months), Transponder (24 months), ELT (12 months).
> **ACS:** [[PA.I.B#^pa-i-b-k1b|PA.I.B.K1b]] — **Stage:** checkride

## Official Text

- **(a)** No persons may use an ATC transponder that is specified in 91.215(a), 121.345(c), or § 135.143(c) of this chapter unless, within the preceding 24 calendar months, the ATC transponder has been tested and inspected and found to comply with appendix F of part 43 of this chapter; and

- **(b)** Following any installation or maintenance on an ATC transponder where data correspondence error could be introduced, the integrated system has been tested, inspected, and found to comply with paragraph (c), appendix E, of part 43 of this chapter.

- **(c)** The tests and inspections specified in this section must be conducted by—

    - **(1)** A certificated repair station properly equipped to perform those functions and holding—

        - **(i)** A radio rating, Class III;

        - **(ii)** A limited radio rating appropriate to the make and model transponder to be tested;

        - **(iii)** A limited rating appropriate to the test to be performed;

    - **(2)** A holder of a continuous airworthiness maintenance program as provided in part 121 or § 135.411(a)(2) of this chapter; or

    - **(3)** The manufacturer of the aircraft on which the transponder to be tested is installed, if the transponder was installed by that manufacturer.

## Source Notes

**Citations:**

\[Docket 18334, 54 FR 34311, Aug. 18, 1989, as amended by Amdt. 91-267, 66 FR 21066, Apr. 27, 2001; Amdt. 91-269, 66 FR 41116, Aug. 6, 2001]

## Explicit Cross-References

- [[135.143|§ 135.143]]
- [[135.411|§ 135.411]]
- [[Part 43]]

## Defined Terms

- [[1.1#^def-aircraft|Aircraft]] (§ 1.1)
- [[1.2#^def-atc|ATC]] (§ 1.2)
- [[1.1#^def-maintenance|Maintenance]] (§ 1.1)
- [[1.1#^def-person|Person]] (§ 1.1)
- [[1.1#^def-rating|Rating]] (§ 1.1)

## Related (derived)

> [!info] Derived links
> Suggested by lexical similarity (`lexical-tfidf` v1), not by an explicit reference. Review in `data/enrichment/related-review.json`.

- [[91.411|§ 91.411 — Altimeter system and altitude reporting equipment tests and inspections]]
- [[107.52|§ 107.52 — ATC transponder equipment prohibition]]
